...
export const analyticsService = {
	async getClassOverview(
		...
		if (!response.ok) {
			throw new Error(
				await parseError(response, "Không thể tải tổng quan lớp"),
			);
		}
	...
	async getWeakTasks(
		...
		if (!response.ok) {
			throw new Error(
				await parseError(response, "Không thể tải danh sách câu yếu"),
			);
		}
	...
	async getProjectPerformance(
		...
		if (!response.ok) {
			throw new Error(
				await parseError(response, "Không thể tải hiệu suất theo dự án"),
			);
		}
