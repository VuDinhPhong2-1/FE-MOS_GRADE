...
const INSERTED_IMAGE_ASSET_API_BASE_URL = `${API_BASE_URL}/inserted-image-assets`;
...
export const insertedImageAssetsService = {
	...
	async upload(
		file: File,
		getAccessToken: GetAccessToken,
	): Promise<InsertedImageAssetUploadResult> {
		...
		if (!response.ok) {
			throw new Error(
				await parseErrorMessage(response, "Tải ảnh lên thất bại."),
			);
		}
	...
	async fetchPreviewUrl(
		...
		const response = await fetch(
			`${INSERTED_IMAGE_ASSET_API_BASE_URL}/${assetId}`,
			{ headers },
		);
