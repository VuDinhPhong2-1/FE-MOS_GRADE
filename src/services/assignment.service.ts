...
const normalizeAssignmentEndpoint = <T extends Assignment>(
	assignment: T,
): T => ({
	...assignment,
...
const extractProjectNumber = (
	assignment: Pick<Assignment, "gradingApiEndpoint" | "name">,
): number | null => {
	const normalizedEndpoint = normalizeProjectEndpoint(
...
const compareAssignmentsForDisplay = <T extends Assignment>(
	left: T,
	right: T,
): number => {
	...
	if (
		leftProjectNumber !== null &&
		rightProjectNumber !== null &&
		leftProjectNumber !== rightProjectNumber
	) {
...
export const assignmentService = {
	...
	async getByClassWithStats(
		...
		return sortAssignmentsForDisplay(
			data.map((item) =>
				normalizeAssignmentEndpoint(item as AssignmentWithStats),
			),
		);
	},
