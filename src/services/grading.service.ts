 ...
 */
const splitSubjectAndProjectCode = (
	gradingEndpoint: string,
): { subject: string; projectCode: string } => {
...
const inferLegacyFileType = (
	...
	if (
		normalizedCode.includes("-excel") ||
		normalizedCode.startsWith("excel/")
	) {
		return "excel";
	}
...
export const gradingService = {
	...
	async gradeByEndpoint(
		...
		const { subject, projectCode } =
			splitSubjectAndProjectCode(gradingEndpoint);
	...
	async gradeProject09(
		...
		return gradingService.gradeByEndpoint(
			"excel/project09",
			studentFile,
			getAccessToken,
			meta,
	...
	async gradeForTesting(
		...
		const { projectNumber, fileType } = normalizeProjectCode(
			projectCode,
			studentFile,
		);
		const formData = new FormData();
