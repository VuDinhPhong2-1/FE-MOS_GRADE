								...
								<DialogHeader className="mb-0 flex-row items-center gap-3 space-y-0 text-left">
									...
									<div>
										...
										<DialogDescription className="text-xs text-m3-on-surface-variant">
											Cấp hoặc thu hồi quyền quản lý lớp học cho các giáo viên
											khác trong hệ thống
										</DialogDescription>
									</div>
								</DialogHeader>
								...
								{isLoadingTeachers ? (
									<div className="flex h-44 flex-col items-center justify-center gap-2 text-m3-on-surface-variant">
										<ProgressIndicator
											...
											shape="wavy"
											size={40}
											aria-label="Đang tải danh sách giáo viên..."
										/>
									...
									<div className="max-h-72 mt-2 overflow-y-auto pr-1">
										...
										>
											{filteredTeachers.map((teacher) => {
												const isOwner =
													teacher.userId === handoverClass.ownerId;
												const granted = Boolean(
													handoverClass.managerTeacherIds?.includes(
														teacher.userId,
													),
												);
														...
														trailingContent={
															isOwner ? (
																...
																<Button
																	...
																	loading={isBusy}
																	onClick={() =>
																		onToggleHandover(teacher.userId, granted)
																	}
																	className={`rounded-full text-xs font-semibold ${
																		granted
																			? "border-m3-error text-m3-error hover:bg-m3-error-container/30"
																			: ""
																	}`}
																>
