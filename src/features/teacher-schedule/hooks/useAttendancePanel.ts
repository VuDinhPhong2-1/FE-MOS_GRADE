	...
	return {
		...
		computerRoom: {
			...
			brokenMachinesDetail:
				...
				attendance.computerRoom.brokenMachinesDetail,
			brokenMachineCount: room.brokenMachineCount,
			totalMachinesText:
				room.totalMachinesText || attendance.computerRoom.totalMachinesText,
			netSupportStatus:
				room.netSupportStatus || attendance.computerRoom.netSupportStatus,
			audioStatus: room.audioStatus || attendance.computerRoom.audioStatus,
			coolingStatus:
				room.coolingStatus || attendance.computerRoom.coolingStatus,
			devicesPoweredOffStatus:
				room.devicesPoweredOffStatus ||
				attendance.computerRoom.devicesPoweredOffStatus,
			seatingOrderStatus:
				room.seatingOrderStatus || attendance.computerRoom.seatingOrderStatus,
			roomHygieneStatus:
				room.roomHygieneStatus || attendance.computerRoom.roomHygieneStatus,
		},
	};
};
...
export const useAttendancePanel = ({
	...
	const [attendanceData, setAttendanceData] =
		useState<ScheduleAttendanceResponse | null>(null);
	const [attendanceDraft, setAttendanceDraft] = useState<
		Record<string, AttendanceDraftState>
	>({});
	const [reportsDraft, setReportsDraft] = useState<ScheduleReportsPayload>(
		emptyReportsPayload(),
	);
	const [attendanceTab, setAttendanceTab] =
		useState<AttendancePanelTab>("attendance");
	...
	const openAttendance = useCallback(
		async (item: ScheduleItem) => {
			try {
				...
				let response = await scheduleService.getAttendance(
					item.id,
					getAccessToken,
				);
				...
				notify.error(
					error instanceof Error ? error.message : "Không thể mở điểm danh",
				);
			} finally {
	...
	const handleSaveAttendance = useCallback(async () => {
		...
		try {
			...
			const payload: SaveScheduleAttendanceItem[] = attendanceData.students.map(
				(student) => ({
			...
			notify.error(
				error instanceof Error
					? error.message
					: "Không thể lưu điểm danh và báo cáo",
			);
		} finally {
	...
	const handleSyncAttendanceToGoogleSheet = useCallback(async () => {
		...
		try {
			...
			const result = await scheduleService.syncAttendanceToGoogleSheet(
				attendanceData.scheduleId,
				getAccessToken,
			);
			...
			notify.error(
				error instanceof Error
					? error.message
					: "Không thể đồng bộ điểm danh với Google Sheet",
			);
		} finally {
	...
	const filteredAttendanceStudents = useMemo(() => {
		...
		if (keyword) {
			list = list.filter((student) => {
				const fullName =
					`${student.middleName} ${student.firstName}`.toLowerCase();
				return (
					fullName.split(/\s+/).includes(keyword) ||
