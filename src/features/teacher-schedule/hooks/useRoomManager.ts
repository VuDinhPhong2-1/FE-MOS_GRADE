...
export const useRoomManager = ({
  ...
  const openRoomManager = useCallback(
    (fallbackSchoolId?: string) => {
      const targetSchoolId = fallbackSchoolId || roomManagerSchoolId || schools[0]?.id || '';
      if (!targetSchoolId) {
  ...
  const handleDeleteRoom = useCallback(
    async (room: ComputerRoom, currentScheduleRoomId?: string, onRoomCleared?: () => void) => {
  ...
  const handleSaveRoom = useCallback(
    ...
    [roomForm, roomManagerSchoolId, editingRoomId, getAccessToken, resetRoomForm, loadRoomManagerRows, onRoomsChanged]
  );
