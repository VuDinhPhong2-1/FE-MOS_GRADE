...
interface StudentTableContextValue {
	readOnly: boolean;
	inlineSavingStudentId: string | null;
	onCompetencyChange: (student: Student, level: CompetencyLevel) => void;
	onExamToggle: (student: Student) => void;
...
const StudentTableContext = createContext<StudentTableContextValue | null>(
	null,
);

const useStudentTableContext = () => {
	const context = useContext(StudentTableContext);
	if (!context) {
		throw new Error(
			"Student table cell components must be used within StudentTableContext.Provider",
		);
	}
	return context;
};
...
const CompetencyCell = ({ row }: { row: { original: Student } }) => {
	const student = row.original;
	const { inlineSavingStudentId, readOnly, onCompetencyChange } =
		useStudentTableContext();
...
const ExamCell = ({ row }: { row: { original: Student } }) => {
	const student = row.original;
	const { inlineSavingStudentId, readOnly, onExamToggle } =
		useStudentTableContext();
	const isSaving = inlineSavingStudentId === student.id;
...
const ActionsCell = ({ row }: { row: { original: Student } }) => {
	const student = row.original;
	const { readOnly, onEdit, onDelete } = useStudentTableContext();
	const isTemp = student.id.startsWith("temp-");

	if (readOnly) {
		return <span className="text-xs text-m3-on-surface-variant">Chỉ xem</span>;
	}
...
const studentTableColumns = helper.columns([
	helper.display({
		id: "index",
		header: "STT",
		meta: {
	...
	helper.accessor("firstName", {
		header: ({ header }) => <SortableHeader header={header} title="Tên" />,
		sortFn: (rowA, rowB) => {
...
const StudentTableComponent = ({
	...
	return (
		<StudentTableContext.Provider value={contextValue}>
			<DataTable
				table={table}
				isLoading={isLoading}
