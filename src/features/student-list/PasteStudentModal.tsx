...
const PasteStudentModalComponent = ({
	...
	const parsedStudents = useMemo(
		() => mapRowsToTempStudents(parsedRows),
		[parsedRows],
	);
	...
	const handlePasteFromClipboard = async () => {
		...
		try {
			...
			setError(
				"Không thể truy cập bộ nhớ tạm. Bạn vui lòng dán thủ công bằng phím tắt Ctrl + V.",
			);
		}
	};
				...
				>
					...
					<DialogFooter className="flex justify-end gap-2 border-t border-m3-outline-variant/40 px-6 py-4">
						...
						<Button
							...
							icon={<Icon name="add" />}
							disabled={
								readOnly || !pasteInput.trim() || parsedStudents.length === 0
							}
