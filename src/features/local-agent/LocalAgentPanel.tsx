...
export function LocalAgentPanel({
	...
	const runAction = async (
		...
		try {
			...
			setError(
				err instanceof Error ? err.message : "Không gọi được Local Agent.",
			);
		} finally {
		...
		try {
			...
			setError(
				err instanceof Error
					? err.message
					: "Không tải được trạng thái Local Agent.",
			);
		} finally {
			setLoadingAction(null);
		}
	...
	// biome-ignore lint/correctness/useExhaustiveDependencies: initial state fetch on mount
	useEffect(() => {
		void refreshState();
	}, []);
	...
	return (
		<section className="mt-6 rounded-lg border border-sky-200 bg-linear-to-br from-sky-50 to-white p-5 shadow-sm">
			...
			<div className="mb-4 grid gap-3 rounded-lg border border-slate-200 bg-white p-4 text-sm md:grid-cols-2">
				<div>
					<span className="font-medium text-slate-700">Publication token:</span>{" "}
					{formatValue(publicationToken)}
				</div>
				<div>
					<span className="font-medium text-slate-700">Student ID:</span>{" "}
					{formatValue(studentId)}
				</div>
				<div>
					<span className="font-medium text-slate-700">Student name:</span>{" "}
					{formatValue(studentName)}
				</div>
				<div>
				...
				<button
					...
					onClick={() =>
						void runAction(
							"force-submit",
							() =>
								localAgentService.submitCurrentProject({
									confirmSaved: true,
									forceSubmit: true,
								}),
			...
			<div className="grid gap-3 rounded-lg border border-slate-200 bg-slate-950/95 p-4 text-sm text-slate-100 md:grid-cols-2">
				...
				<div>
					<span className="font-medium text-slate-300">Project code:</span>{" "}
					{formatValue(state.projectCode)}
				</div>
				<div>
				...
				<div>
					<span className="font-medium text-slate-300">Template file:</span>{" "}
					{formatValue(state.templateFileName)}
				</div>
				<div>
					<span className="font-medium text-slate-300">Current project:</span>{" "}
					{state.currentProjectNumber || 0}/{state.totalProjectCount || 0}
				</div>
				<div>
					<span className="font-medium text-slate-300">Working file:</span>{" "}
					{formatValue(state.workingFilePath)}
				</div>
				<div>
					<span className="font-medium text-slate-300">Is busy:</span>{" "}
					{state.isBusy ? "Yes" : "No"}
				</div>
				<div>
				...
				<div>
					<span className="font-medium text-slate-300">Is completed:</span>{" "}
					{state.isCompleted ? "Yes" : "No"}
				</div>
				<div>
					<span className="font-medium text-slate-300">Last error:</span>{" "}
					{formatValue(state.lastError)}
				</div>
			</div>
		</section>
	);
