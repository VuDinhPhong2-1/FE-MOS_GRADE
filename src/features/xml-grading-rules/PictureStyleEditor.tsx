...
const stylePresetOptions = [
	{
		value: "simpleFrameBlack",
		label: "Simple Frame, Black - khung don vien den",
	},
...
const PictureStyleEditor = ({
	...
	const handleFileChange = async (
		...
		if (!ACCEPTED_IMAGE_TYPES.includes(file.type)) {
			setError(
				"File khong hop le. Vui long chon PNG, JPG, GIF, BMP hoac WebP.",
			);
			event.target.value = "";
			return;
		}
		...
		<div className="mt-4 rounded-2xl border border-violet-100 bg-violet-50/40 p-4">
				...
				<div>
					...
					{previewUrl ? (
						<div className="relative flex h-36 items-center justify-center overflow-hidden rounded-xl border border-slate-200 bg-white p-2">
							<img
								src={previewUrl}
								alt="Bản xem trước kiểu định dạng"
			...
			<div className="mt-4 grid gap-3 md:grid-cols-2">
				...
				<div className="rounded-xl border border-amber-100 bg-amber-50 px-3 py-2 text-[11px] leading-5 text-amber-800">
					Simple Frame, Black se fail neu anh co shadow/effect, nen Moderate
					Frame, Black khong duoc tinh dung.
				</div>
				<label className="text-xs font-semibold text-slate-600">
					Source file
					<input
				...
				<label className="text-xs font-semibold text-slate-600">
					...
					<input
						...
						onChange={(e) =>
							patchConfig({
								targetImageIndex: e.target.value
