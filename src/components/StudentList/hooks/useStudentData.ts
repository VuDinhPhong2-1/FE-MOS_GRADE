...
import { VALID_STATUSES, isStudentActive } from '../types';
...
export const useStudentData = ({
  ...
  const handleInlineCompetencyChange = useCallback(
    async (student: Student, level: CompetencyLevel) => {
      ...
      const status = VALID_STATUSES.includes((student.status || '') as (typeof VALID_STATUSES)[number])
        ...
        setStudents((prev) =>
          prev.map((item) =>
            item.id === student.id
              ? {
                ...item,
                competencyLevel: (updatedStudent.competencyLevel ?? level) as CompetencyLevel,
  ...
  const handleInlineExamToggle = useCallback(
    async (student: Student) => {
      ...
      try {
        ...
        alert(err instanceof Error ? err.message : 'Không thể cập nhật trạng thái thi.');
      } finally {
        setInlineSavingStudentId(null);
      }
    },
