...
export default function AuthPage() {
	...
	const getPostLoginPath = (data: LoginResponse) => {
		if (
			...
			(data.role === "Teacher" &&
				(data.teacherApprovalStatus === "Pending" ||
					data.teacherApprovalStatus === "Rejected"))
		) {
	...
	const handleSubmit = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
		...
		try {
			...
			if (isLogin) {
				...
				notify.success(
					"Tài khoản đã tạo, đang chờ Admin duyệt quyền giáo viên.",
				);
				setIsLogin(true);
					...
					<form onSubmit={handleSubmit} className="space-y-4">
						...
						{isLogin && hasGoogleClientId && (
							<>
								...
								<div className="relative flex w-full justify-center">
									{isGoogleSubmitting && (
										<div className="absolute inset-0 z-10 flex items-center justify-center gap-2 rounded-full bg-m3-surface/85 backdrop-blur-xs text-xs font-semibold text-m3-on-surface">
											<ProgressIndicator
												variant="circular"
												shape="wavy"
												size={18}
									...
									<div
										className={`flex w-full justify-center transition-opacity ${isAuthBusy ? "pointer-events-none opacity-60" : ""}`}
									>
										<GoogleLogin
											...
											useOneTap={false}
											theme={
												effectiveMode === "dark" ? "filled_black" : "outline"
											}
											size="large"
											shape="pill"
											text="continue_with"
						...
						{isLogin && !hasGoogleClientId && (
							<div className="rounded-2xl border border-amber-300 bg-amber-50 dark:bg-amber-950/30 p-3 text-xs text-amber-800 dark:text-amber-200">
								Đăng nhập Google chưa được cấu hình. Đặt biến{" "}
								<code>VITE_GOOGLE_CLIENT_ID</code> trong file <code>.env</code>.
							</div>
						)}
					</form>
