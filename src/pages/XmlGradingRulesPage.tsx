  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div><h1 className="text-2xl font-bold text-slate-900">Chấm điểm mới bằng XML Rules</h1><p className="text-sm text-slate-500">Quản lý bộ luật → bài project → câu/task → điều kiện chấm và test chấm file Office.</p></div>
        <button onClick={() => setSelected(emptyRuleSet())} className="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700"><Plus size={16} /> Tạo bộ luật mới</button>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[360px_1fr]">
        <aside className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="mb-3 flex gap-2">
            ...
            <select value={activeFilter} onChange={(e) => setActiveFilter(e.target.value as 'all' | 'true' | 'false')} className="rounded-lg border px-2 text-sm"><option value="all">Tất cả</option><option value="true">Đang bật</option><option value="false">Đang tắt</option></select>
        ...
        <main className="space-y-4">
          <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="grid gap-3 md:grid-cols-4">
              <label className="text-sm">Môn / loại file (subject)<input value={selected.subject} onChange={(e) => replaceSelected({ ...selected, subject: e.target.value })} placeholder="excel hoặc word" className="mt-1 w-full rounded-lg border px-3 py-2" /></label>
              <label className="text-sm">Phiên bản bộ luật (version)<input value={selected.version} onChange={(e) => replaceSelected({ ...selected, version: e.target.value })} placeholder="v1, 2026-08..." className="mt-1 w-full rounded-lg border px-3 py-2" /></label>
              <label className="flex items-center gap-2 pt-7 text-sm"><input type="checkbox" checked={selected.isActive} onChange={(e) => replaceSelected({ ...selected, isActive: e.target.checked })} /> Bật dùng để chấm (Active)</label>
                    ...
                    {project.tasks.map((task, ti) => (
                      <div key={`${task.taskId}-${ti}`} className="rounded-lg border bg-slate-50 p-3">
                        <div className="grid gap-2 md:grid-cols-[1fr_1fr_100px_auto]">
                          ...
                          <label className="text-xs font-medium text-slate-600">Điểm tối đa<input type="number" value={task.maxScore} onChange={(e) => mutateTask(pi, ti, { maxScore: Number(e.target.value) })} className="mt-1 w-full rounded border px-2 py-1 text-sm font-normal text-slate-900" /></label>
                          <button onClick={() => mutateProject(pi, { tasks: project.tasks.filter((_, i) => i !== ti) })} title="Xóa câu/task" className="self-end pb-2 text-red-600"><Trash2 size={16} /></button>
                        </div>
                          ...
                          {task.conditions.map((condition, ci) => (
                            <div key={`${condition.conditionId}-${ci}`} className="rounded-lg border bg-white p-3">
                              <div className="grid gap-2 md:grid-cols-[1fr_90px_1fr_170px_140px_auto]">
                                ...
                                <label className="text-xs font-medium text-slate-600">File XML cần kiểm tra (sourceFile)<input value={condition.sourceFile} onChange={(e) => mutateCondition(pi, ti, ci, { sourceFile: e.target.value })} placeholder="Ví dụ xl/worksheets/sheet1.xml" className="mt-1 w-full rounded border px-2 py-1 text-sm font-normal text-slate-900" /></label>
                                <label className="text-xs font-medium text-slate-600">Cách so khớp (compareMode)<select value={condition.compareMode} onChange={(e) => mutateCondition(pi, ti, ci, { compareMode: e.target.value as XmlCompareMode })} title="Cách so khớp XML/chuỗi" className="mt-1 w-full rounded border px-2 py-1 text-sm font-normal text-slate-900">{compareModes.map((m) => <option key={m} value={m}>{m === 'xmlContainsNormalized' ? 'So khớp XML đã chuẩn hóa' : m === 'xmlContains' ? 'So khớp XML chứa đoạn này' : m === 'xmlEquivalentWholeFile' ? 'So toàn bộ file XML' : 'So chuỗi chính xác'}</option>)}</select></label>
                                <label className="text-xs font-medium text-slate-600">Quy tắc nhiều giá trị<select value={condition.matchPolicy} onChange={(e) => mutateCondition(pi, ti, ci, { matchPolicy: e.target.value as XmlMatchPolicy })} title="Quy tắc khi có nhiều expected value" className="mt-1 w-full rounded border px-2 py-1 text-sm font-normal text-slate-900">{matchPolicies.map((m) => <option key={m} value={m}>{m === 'all' ? 'Phải đúng tất cả' : m === 'any' ? 'Đúng một trong các dòng' : 'Đúng theo thứ tự'}</option>)}</select></label>
                                <button onClick={() => mutateTask(pi, ti, { conditions: task.conditions.filter((_, i) => i !== ci) })} title="Xóa điều kiện" className="self-end pb-2 text-red-600"><Trash2 size={16} /></button>
                              </div>
                              <label className="mt-2 block text-xs font-medium text-slate-600">Giá trị cần tìm trong XML (expectedValue / expectedValues)<textarea value={expectedText(condition)} onChange={(e) => mutateCondition(pi, ti, ci, { expectedValues: e.target.value.split('\n') })} placeholder="Mỗi dòng là một đoạn XML/chuỗi cần kiểm tra" className="mt-1 min-h-24 w-full rounded border px-2 py-1 font-mono text-xs font-normal text-slate-900" /></label>
                              <div className="mt-2 grid gap-2 md:grid-cols-3">
                                <label className="text-xs font-medium text-slate-600">Thông báo khi đúng<input value={condition.feedback?.successDetail || ''} onChange={(e) => mutateCondition(pi, ti, ci, { feedback: { ...condition.feedback, successDetail: e.target.value } })} placeholder="successDetail" className="mt-1 w-full rounded border px-2 py-1 text-sm font-normal text-slate-900" /></label>
                                <label className="text-xs font-medium text-slate-600">Thông báo khi sai<input value={condition.feedback?.errorMessage || ''} onChange={(e) => mutateCondition(pi, ti, ci, { feedback: { ...condition.feedback, errorMessage: e.target.value } })} placeholder="errorMessage" className="mt-1 w-full rounded border px-2 py-1 text-sm font-normal text-slate-900" /></label>
                                <label className="text-xs font-medium text-slate-600">Gợi ý cách sửa<input value={condition.feedback?.fixAction || ''} onChange={(e) => mutateCondition(pi, ti, ci, { feedback: { ...condition.feedback, fixAction: e.target.value } })} placeholder="fixAction" className="mt-1 w-full rounded border px-2 py-1 text-sm font-normal text-slate-900" /></label>
                              </div>
                            </div>
                          ))}
                        </div>
