...
import { GradingWorkspace } from "../features/grading";
...
const ClassGradingPage = () => {
	...
	const locationState =
		(location.state as ClassGradingLocationState | null) ?? null;
	...
	if (isLoading) {
		return <RouteLoadingFallback message="Đang tải màn hình chấm điểm..." />;
	}
		...
		return (
			<div className="space-y-4">
				<div className="flex items-center gap-3 rounded-2xl bg-amber-500/15 p-4 text-xs font-medium text-amber-700 dark:text-amber-300 shadow-xs">
					...
					<span>
						Bạn chỉ có quyền xem lớp này, không có quyền thực hiện thao tác chấm
						điểm.
					</span>
				</div>
			</div>
