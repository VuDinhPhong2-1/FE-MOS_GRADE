...
const normalizeAgentState = (
	...
	return {
		...state,
		workingFileExists:
			state.workingFileExists ?? Boolean(state.workingFilePath),
		hasRecoverableSession:
			state.hasRecoverableSession ??
			Boolean(state.sessionId && state.publicationToken),
...
export function PublicExamPage() {
	...
	const [publication, setPublication] =
		useState<PublicExamPublicationInfo | null>(null);
			...
			try {
				...
				setError(
					err instanceof Error
						? err.message
						: "Khong the tai thong tin ca thi.",
				);
			} finally {
				if (active) {
					setLoading(false);
	...
	const selectedStudent = useMemo(
		() =>
			publication?.students.find(
				(student) => student.id === selectedStudentId,
			) || null,
		[publication, selectedStudentId],
	);

	const agentStateMatchesToken = Boolean(
		agentState?.sessionId && agentState.publicationToken === token,
	);
	...
	const displayStudentName =
		selectedStudent?.fullName || agentState?.studentName || "";
	...
	const handleRefreshAgentState = () =>
		void runAgentAction(
			"refresh",
			() => localAgentService.getCurrentState(),
			"Da lam moi trang thai.",
		);
						...
						{loading && (
							<div className="flex items-center gap-3 rounded-xl border border-sky-100 bg-sky-50 px-4 py-3 text-sm text-sky-800">
								<ProgressIndicator
									...
									showTrack
									size={16}
									aria-label="Đang tải thông tin ca thi..."
								/>
							...
							<>
									...
									{canResumeCurrentStudent && (
										<button
											...
											onClick={
												needsRecreateCurrentStudent
													? handleRecreateCurrentProject
													: handleRestartCurrentProject
											}
											disabled={loadingAction !== null}
										...
										>
											...
											{needsRecreateCurrentStudent
												? loadingAction === "recreate"
													? "Dang tao lai file..."
													: "Tao lai file project hien tai"
												: loadingAction === "restart"
													? "Dang lam lai project..."
													: "Lam lai project hien tai"}
										</button>
									)}
								...
								{!selectedStudent && (
									<div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 px-4 py-4 text-sm text-slate-600">
										Chon dung ten cua ban roi bam Start exam de mo bai thi tren
										Local Agent.
									</div>
								)}
							</>
