...
const AssignmentManagementPage = ({
	...
	const [isCreatingExamPublication, setIsCreatingExamPublication] =
		useState(false);
	...
	const [createExamPublicationMessage, setCreateExamPublicationMessage] =
		useState<string | null>(null);
	const [localAgentPublicationToken, setLocalAgentPublicationToken] =
		useState("");
	...
	const [quickPracticeCode, setQuickPracticeCode] =
		useState<QuickPracticeCode>("practice01");
	const [quickAssignmentDrafts, setQuickAssignmentDrafts] = useState<
		QuickAssignmentDraft[]
	>([]);
	const [quickAssignmentDescription, setQuickAssignmentDescription] =
		useState("");
	const [isQuickCreatingAssignments, setIsQuickCreatingAssignments] =
		useState(false);
	const [selectedStudentIds, setSelectedStudentIds] = useState<string[]>([]);
	const [selectWholeClass, setSelectWholeClass] = useState(true);
	const [selectedExamAssignmentIds, setSelectedExamAssignmentIds] = useState<
		string[]
	>([]);
	...
	const [isAssignmentListPanelOpen, setIsAssignmentListPanelOpen] =
		useState(true);
	const [isAssignmentFormPanelOpen, setIsAssignmentFormPanelOpen] =
		useState(false);
	...
	const publishableExamAssignments = useMemo(
		() => assignments.filter(isAssignmentPublishableForExam),
		[assignments],
	);
	...
	const selectedExamAssignments = useMemo(
		() =>
			selectedExamAssignmentIds
				.map(
					(assignmentId) =>
						publishableExamAssignments.find(
	...
	// biome-ignore lint/correctness/useExhaustiveDependencies: Initial data load on mount
	useEffect(() => {
		void loadClasses();
		void loadGradingEndpoints();
	}, [getAccessToken]);

	// biome-ignore lint/correctness/useExhaustiveDependencies: Reload data when class or inactive filter changes
	useEffect(() => {
		void loadAssignments(selectedClassId);
		void loadStudents(selectedClassId);
	...
	useEffect(() => {
		setSelectedExamAssignmentIds((current) =>
			current.filter((assignmentId) =>
				publishableExamAssignments.some(
					(assignment) => assignment.id === assignmentId,
				),
			),
		);
	...
	const handleEndpointChange = (endpointValue: string) => {
		const endpoint = gradingEndpoints.find(
	...
	const createAssignmentsFromQuickDrafts = async (
		drafts: QuickAssignmentDraft[],
		practiceCode: QuickPracticeCode,
	) => {
		if (!selectedClassId) {
		...
		if (invalidDraft) {
			setActionMessage(
				`Ten bai tap cho ${invalidDraft.displayName} khong duoc de trong.`,
			);
			return;
		}
			...
			for (const draft of selectedDrafts) {
				try {
					...
					failedAssignments.push(
						`${draft.displayName}: ${error instanceof Error ? error.message : "Khong the tao bai tap."}`,
					);
				}
			}
	...
	const handleCreateSelectedQuickAssignments = async () => {
		await createAssignmentsFromQuickDrafts(
			quickAssignmentDrafts,
			quickPracticeCode,
		);
	};
	...
	const handleQuickCreateByPractice = async (
		...
		const endpoints = resolveQuickEndpoints(
			gradingEndpoints,
			quickSubject,
			practiceCode,
		);
		...
		if (drafts.length === 0) {
			setActionMessage(
				`Khong co project ${practice?.label || practiceCode} cho mon ${quickSubject.toUpperCase()}.`,
			);
			return;
		}
	...
	const startEdit = (assignment: Assignment) => {
		...
		setForm({
			...
			projectCode:
				assignment.projectCode ||
				getProjectCodeFromEndpoint(assignment.gradingApiEndpoint),
	...
	const handleSubmit = async (event: FormEvent) => {
		...
		try {
			...
			setActionMessage(
				error instanceof Error ? error.message : "Không thể lưu bài tập.",
			);
		} finally {
	...
	const handleToggleActive = async (assignment: Assignment) => {
		...
		try {
			...
			setActionMessage(
				assignment.isActive ? "Đã lưu trữ bài tập." : "Đã khôi phục bài tập.",
			);
			await loadAssignments(selectedClassId);
		} catch (error) {
			setActionMessage(
	...
	const handleToggleStudent = (studentId: string) => {
		setSelectedStudentIds((current) => {
			...
			setSelectWholeClass(
				students.length > 0 && nextValue.length === students.length,
			);
			return nextValue;
		});
	};
	...
	const handleCreateLocalAgentExam = async () => {
		...
		try {
			...
			setLocalAgentPublicationToken("");
			setCreateExamPublicationMessage(
				error instanceof Error ? error.message : "Không thể tạo ca thi test.",
			);
		} finally {
	...
	usePageHeader(
		{
			...
			subtitle:
				"Tạo, chỉnh sửa, lưu trữ hoặc xóa bài tập cho từng lớp theo chuẩn MOS",
			actions: [
				{
			...
			{showFiltersPanel && (
				...
				>
					<summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-6 py-4">
						...
						<Icon
							name="expand_more"
							className="text-xl text-m3-on-surface-variant transition-transform duration-200 group-open:rotate-180"
						/>
					</summary>
					...
					<div className="border-t border-m3-outline-variant/40 px-6 pb-6 pt-5">
						...
						<div className="mt-4 flex flex-wrap items-center gap-3 text-xs text-m3-on-surface-variant">
							...
							<span className="hidden text-m3-outline-variant sm:inline">
								•
							</span>
							<span>
								Đang hoạt động:{" "}
					...
					<summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-6 py-4">
						...
						<div className="flex items-center gap-3">
							<span className="hidden rounded-full bg-m3-primary/10 px-3 py-1 text-xs font-semibold text-m3-primary sm:inline">
								...
								{selectedExamAssignments.length} bài tập
							</span>
							<Icon
								name="expand_more"
								className="text-xl text-m3-on-surface-variant transition-transform duration-200 group-open:rotate-180"
					...
					<div className="border-t border-m3-outline-variant/40 px-6 pb-6 pt-5">
						<div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
							<div>
								...
								<h2 className="mt-3 text-lg font-bold text-m3-on-surface">
									Tạo ca thi từ bài tập của lớp
								</h2>
								<p className="mt-1 text-xs text-m3-on-surface-variant">
									Chọn học sinh và bài tập đã publish để tạo token cho Local
									Agent / trang thi.
								</p>
							</div>
							<div className="rounded-2xl bg-m3-surface-container-high px-4 py-2 text-xs text-m3-on-surface-variant shadow-xs">
								...
								học sinh •{" "}
								<strong className="text-m3-on-surface font-bold">
									{selectedExamAssignments.length}
								</strong>{" "}
								bài tập
							</div>
						</div>
						...
						<section className="mt-5 rounded-3xl bg-m3-surface-container-low p-4 shadow-xs">
							<div className="grid gap-4 lg:grid-cols-[1fr_280px] lg:items-end">
								<div>
									...
									<p className="mt-1 text-xs text-m3-on-surface-variant">
										Học sinh và bài tập đưa vào ca thi sẽ lấy từ lớp đang chọn ở
										đây.
									</p>
								</div>
									...
									<div className="flex items-center justify-between gap-3">
										...
										<label className="inline-flex items-center gap-2 text-xs text-m3-on-surface-variant">
											<input
												...
												onChange={(event) =>
													handleToggleWholeClass(event.target.checked)
												}
												className="h-4 w-4 rounded-sm border-m3-outline-variant text-m3-primary focus:ring-m3-primary"
										...
										{isLoadingStudents ? (
											...
											<div className="grid gap-2 sm:grid-cols-2">
												{students.map((student) => (
													<label
														key={student.id}
														className="flex items-center gap-2 rounded-xl p-1.5 text-xs hover:bg-m3-surface-container transition-colors"
													>
														<input
															type="checkbox"
														...
														<span className="truncate text-m3-on-surface">
															{buildStudentDisplayName(student) || student.id}
														</span>
													</label>
												))}
											</div>
								...
								<div>
									...
									<div className="mt-2 max-h-56 overflow-auto rounded-2xl border border-m3-outline-variant/60 bg-m3-surface p-3">
										{isLoadingAssignments ? (
											...
											<p className="text-xs text-m3-on-surface-variant">
												Lớp này chưa có bài tập đang hoạt động, đã publish và có
												endpoint chấm điểm.
											</p>
										) : (
											<div className="space-y-2">
												{publishableExamAssignments.map((assignment) => (
													<label
														key={assignment.id}
														className="flex items-start gap-2 rounded-xl p-1.5 text-xs hover:bg-m3-surface-container transition-colors"
													>
														<input
															type="checkbox"
															checked={selectedExamAssignmentIds.includes(
																assignment.id,
															)}
															onChange={() =>
																handleToggleExamAssignment(assignment.id)
															}
															className="mt-0.5 h-4 w-4 rounded-sm border-m3-outline-variant text-m3-primary focus:ring-m3-primary"
														/>
														<span>
															<span className="block font-semibold text-m3-on-surface">
																{assignment.name}
															</span>
															<span className="block text-[11px] text-m3-on-surface-variant">
																{assignment.projectCode ||
																	getProjectCodeFromEndpoint(
																		assignment.gradingApiEndpoint,
																	) ||
																	"—"}{" "}
																• {assignment.gradingApiEndpoint}
									...
									<div className="rounded-2xl border border-emerald-300 bg-emerald-50 dark:bg-emerald-950/30 p-3 text-xs text-emerald-800 dark:text-emerald-200">
										...
										<p className="mt-1">
											...
											<a
												...
												href={`/exam/${localAgentPublicationToken}`}
												target="_blank"
												rel="noreferrer"
											>
			...
			<div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_420px]">
				{showAssignmentListPanel && (
					<details
						...
						onToggle={(event) =>
							setIsAssignmentListPanelOpen(event.currentTarget.open)
						}
						className="group rounded-3xl bg-m3-surface-container shadow-xs"
					>
						<summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-6 py-4">
						...
						<div className="border-t border-m3-outline-variant/40">
							...
							{isLoadingAssignments ? (
								...
								<div className="px-6 py-12 text-center">
									...
									<h3 className="mt-3 text-sm font-bold text-m3-on-surface">
										Chưa có bài tập nào
									</h3>
									<p className="mt-1 text-xs text-m3-on-surface-variant">
										Tạo bài tập mới bằng form bên phải để giáo viên có thể quản
										lý và chấm điểm.
									</p>
								</div>
							) : (
											...
											{filteredAssignments.map((assignment) => (
												...
												>
													<td className="px-4 py-2">
														<div
															className="truncate font-semibold text-m3-on-surface"
													...
													<td className="px-4 py-2 text-xs font-medium text-m3-on-surface-variant">
														{subjectLabels[assignment.subject]}
													</td>
													<td className="px-4 py-2">
														<div
															className="truncate text-xs font-medium text-m3-on-surface"
															title={examTypeLabels[assignment.examType]}
														>
													...
													<td className="px-4 py-2 font-bold text-m3-on-surface">
														{assignment.maxScore}
													</td>
													<td className="px-4 py-2">
														<div
														...
														<div
															className="truncate text-[11px] text-m3-on-surface-variant"
															title={
													...
													<td className="px-4 py-2">
														<div className="flex justify-end gap-1">
															...
															<button
																type="button"
																onClick={() =>
																	void handleToggleActive(assignment)
																}
				...
				{showAssignmentFormPanel && (
					<details
						...
						onToggle={(event) =>
							setIsAssignmentFormPanelOpen(event.currentTarget.open)
						}
						className="group rounded-3xl bg-m3-surface-container shadow-xs"
					>
						<summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-6 py-4">
							<div>
								...
								<p className="mt-1 text-xs text-m3-on-surface-variant">
									{editingAssignment
										? "Đang sửa bài tập đã chọn."
										: "Mở khi cần tạo bài tập mới cho lớp."}
								</p>
							</div>
							<Icon
								name="expand_more"
						...
						<form
							...
							className="border-t border-m3-outline-variant/40 p-6"
						>
							...
							<section className="mt-4 rounded-3xl bg-m3-surface-container-low p-4 shadow-xs">
								<div className="grid gap-4 lg:grid-cols-[1fr_280px] lg:items-end">
									<div>
										...
										<p className="mt-1 text-xs text-m3-on-surface-variant">
											Tất cả bài tạo nhanh và bài tạo thủ công bên dưới sẽ được
											lưu vào lớp đang chọn ở đây.
										</p>
									</div>
								...
								<section className="mt-5 rounded-3xl border border-emerald-500/20 bg-emerald-500/5 p-4">
									<div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
										<div>
											...
											<p className="mt-1 text-xs text-emerald-700 dark:text-emerald-400">
												Chọn môn và phần, hệ thống sẽ điền sẵn danh sách project
												để tạo hàng loạt.
											</p>
										</div>
										<div className="grid gap-2 sm:grid-cols-2 lg:min-w-90">
											<label className="block">
												...
												<select
													...
													onChange={(event) =>
														setQuickSubject(
															event.target.value as QuickSubjectCode,
														)
													}
											...
											<label className="block">
												...
												<select
													...
													onChange={(event) =>
														setQuickPracticeCode(
															event.target.value as QuickPracticeCode,
														)
													}
													className="mt-1 w-full rounded-2xl border border-emerald-500/30 bg-m3-surface px-3 py-2 text-xs text-m3-on-surface outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20"
													disabled={isQuickCreatingAssignments}
										...
										<div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
											<div>
												<p className="text-xs font-bold text-m3-on-surface">
													Tạo nhanh từng phần ({quickSubject.toUpperCase()})
												</p>
												<p className="mt-0.5 text-[11px] text-m3-on-surface-variant">
													Bấm một phần để tạo ngay toàn bộ project trong phần
													đó.
												</p>
											</div>
											<div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
												{quickPracticeOptions.map((practice) => {
													const endpointCount = resolveQuickEndpoints(
														gradingEndpoints,
														quickSubject,
														practice.code,
													).length;
													...
													return (
														<button
															...
															type="button"
															onClick={() =>
																void handleQuickCreateByPractice(practice.code)
															}
															disabled={
																isQuickCreatingAssignments ||
																!selectedClassId ||
									...
									<label className="mt-4 block">
										...
										<textarea
											...
											onChange={(event) =>
												setQuickAssignmentDescription(event.target.value)
											}
											rows={2}
									...
									<div className="mt-4 rounded-2xl border border-emerald-500/20 bg-m3-surface">
										<div className="flex flex-col gap-3 border-b border-m3-outline-variant/40 p-3 lg:flex-row lg:items-center lg:justify-between">
											<div>
												...
												<p className="mt-0.5 text-[11px] text-m3-on-surface-variant">
													Có thể bỏ chọn project không cần tạo hoặc đổi tên từng
													bài.
												</p>
											</div>
															...
															<tr key={draft.endpoint}>
																...
																<td className="px-3 py-2">
																	<input
																		...
																		value={draft.name}
																		onChange={(event) =>
																			handleQuickAssignmentNameChange(
																				draft.endpoint,
																				event.target.value,
																			)
																		}
																...
																<td className="px-3 py-2 text-right font-bold text-m3-on-surface">
																	{draft.maxScore}
																</td>
															</tr>
														))}
										...
										<div className="border-t border-m3-outline-variant/40 p-3">
											<Button
												...
												type="button"
												onClick={() =>
													void handleCreateSelectedQuickAssignments()
												}
												disabled={
													isQuickCreatingAssignments ||
													!selectedClassId ||
													selectedQuickAssignmentCount === 0
												}
											...
											>
												<div className="flex items-center justify-center gap-2">
													...
													<span>
														{isQuickCreatingAssignments
															? "Đang tạo nhanh..."
															: `Tạo nhanh ${selectedQuickAssignmentCount} bài đã chọn`}
													</span>
												</div>
											</Button>
										</div>
								...
								<div className="grid gap-4 sm:grid-cols-2">
									<label className="block">
										...
										<select
											value={form.subject}
											onChange={(event) =>
												updateForm("subject", event.target.value as SubjectCode)
											}
											className="mt-1.5 w-full rounded-2xl border border-m3-outline-variant bg-m3-surface px-3.5 py-2.5 text-sm text-m3-on-surface outline-none transition focus:border-m3-primary focus:ring-2 focus:ring-m3-primary/20"
										>
									...
									<label className="block">
										...
										<select
											...
											onChange={(event) =>
												updateForm(
													"examType",
													event.target.value as ExamTypeCode,
												)
											}
											className="mt-1.5 w-full rounded-2xl border border-m3-outline-variant bg-m3-surface px-3.5 py-2.5 text-sm text-m3-on-surface outline-none transition focus:border-m3-primary focus:ring-2 focus:ring-m3-primary/20"
										>
								...
								<label className="block">
									...
									<select
										...
										onChange={(event) =>
											updateForm(
												"gradingType",
												event.target.value as GradingTypeCode,
											)
										}
										className="mt-1.5 w-full rounded-2xl border border-m3-outline-variant bg-m3-surface px-3.5 py-2.5 text-sm text-m3-on-surface outline-none transition focus:border-m3-primary focus:ring-2 focus:ring-m3-primary/20"
									>
